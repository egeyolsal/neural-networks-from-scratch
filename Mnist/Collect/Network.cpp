...
{
    float total_err;
    // This array will be returned to Main for the Chart
    ...
    int& epoch)
{
    ...
    int input_dim = layer_sizes[0]; // 784

    // the output size is equal to the input size.
    int output_dim = layer_sizes[num_layers_total - 1];

    float* desired = new float[output_dim];
    epoch = 0;
    ...
    do {
        ...
        for (int step = 0; step < numSamples; step++) {
            ...
            // Pointer to the input layer
            float* InputLayer = &all_fnet[0];

            for (int k = 0; k < output_dim; k++) {
                // The output must be the same as the input.
                desired[k] = InputLayer[k];
        ...
        total_err /= float(output_dim * numSamples);
        error_history[epoch] = total_err;
        epoch++;
